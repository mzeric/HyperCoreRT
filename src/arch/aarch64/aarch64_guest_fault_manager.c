...
static struct aarch64_guest_fault_result map_stage2_page(vcpu_t *vcpu, paddr_t ipa) {
    if (!vcpu)
        return aarch64_guest_fault_failure(AARCH64_GUEST_FAULT_NO_CURRENT_VCPU);

    struct mem_region *mem = guest_mem_find_region(vcpu, ipa, 0);
    if (!mem) {
        safe_printf("guest stage2 fault: unmapped ipa=%lx\n", ipa);
        return aarch64_guest_fault_failure(AARCH64_GUEST_FAULT_UNMAPPED_IPA);
    }
    ...
    return aarch64_guest_fault_resume_guest();
}

static struct aarch64_guest_fault_result emulate_mmio_access(vcpu_t *vcpu,
    ...
    if (!mem->dev || !mem->dev->driver || !mem->dev->driver->ops)
        return aarch64_guest_fault_failure(AARCH64_GUEST_FAULT_NON_MMIO_ACCESS_FAULT);

    if (esr->dabt.write && !mem->dev->driver->ops->write)
        return aarch64_guest_fault_failure(AARCH64_GUEST_FAULT_EMULATE_FAILED);
...
static struct aarch64_guest_fault_result handle_sysreg(struct cpu_user_regs *regs,
                                                       const union esr *esr) {
    if (!current_task())
        return aarch64_guest_fault_failure(AARCH64_GUEST_FAULT_NO_CURRENT_TASK);
    if (!current_vcpu())
        return aarch64_guest_fault_failure(AARCH64_GUEST_FAULT_NO_CURRENT_VCPU);
    ...
    if (ret != 0)
        return aarch64_guest_fault_failure(AARCH64_GUEST_FAULT_SYSREG_EMULATE_FAILED);
...
static struct aarch64_guest_fault_result handle_instruction_abort(
    ...
    return aarch64_guest_fault_failure(AARCH64_GUEST_FAULT_UNSUPPORTED_INSTRUCTION_ABORT);
}

static struct aarch64_guest_fault_result handle_data_abort(
    struct cpu_user_regs *regs, const union esr *esr) {
    ...
    print_iss_detail(*esr);
    return aarch64_guest_fault_failure(AARCH64_GUEST_FAULT_UNSUPPORTED_DATA_ABORT);
}

static struct aarch64_guest_fault_result handle_hvc(struct cpu_user_regs *regs) {
    if (!current_task())
        return aarch64_guest_fault_failure(AARCH64_GUEST_FAULT_NO_CURRENT_TASK);
    if (!current_vcpu())
        return aarch64_guest_fault_failure(AARCH64_GUEST_FAULT_NO_CURRENT_VCPU);
    ...
    return aarch64_guest_fault_yield_scheduler();
}

struct aarch64_guest_fault_result aarch64_guest_fault_handle_exception(
    struct cpu_user_regs *regs, const union esr *esr) {
    switch (esr->ec) {
    case HSR_EC_SYSREG:
    ...
    default:
        return aarch64_guest_fault_failure(AARCH64_GUEST_FAULT_UNSUPPORTED_EXCEPTION_CLASS);
    }
}
