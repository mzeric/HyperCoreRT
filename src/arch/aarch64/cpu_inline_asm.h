...
#define rev64(val)		({ u32 d1, d2; \
				d1 = (u32)((uint64_t)val >> 32); d2 = (u32)val; \
...
#define cpu_supports_address_auth_arch() ({ uint64_t isar1; \
				   asm volatile("mrs %0, id_aa64isar1_el1" \
						: "=r"(isar1)); \
				   (isar1 & ID_AA64ISAR1_APA_MASK); \
				})

#define cpu_supports_address_auth_imp() ({ uint64_t isar1; \
				   asm volatile("mrs %0, id_aa64isar1_el1" \
						: "=r"(isar1)); \
				   (isar1 & ID_AA64ISAR1_API_MASK); \
				})

#define cpu_supports_thumbee()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_pfr0_el1" \
						: "=r"(pfr0)); \
				   (pfr0 & ID_PFR0_THUMBEE_MASK); \
				})

#define cpu_supports_thumb()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_pfr0_el1" \
						: "=r"(pfr0)); \
				   (pfr0 & ID_PFR0_THUMBEE_MASK); \
				})

#define cpu_supports_thumb2()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_pfr0_el1" \
						: "=r"(pfr0)); \
				   ((pfr0 & ID_PFR0_THUMB_MASK) == ID_PFR0_THUMB2_MASK); \
				})

#define cpu_supports_jazelle()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_pfr0_el1" \
						: "=r"(pfr0)); \
				   (pfr0 & ID_PFR0_JAZELLE_MASK); \
				})

#define cpu_supports_arm()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_pfr0_el1" \
						: "=r"(pfr0)); \
				   (pfr0 & ID_PFR0_ARM_MASK); \
				})

#define cpu_supports_asimd()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_aa64pfr0_el1" \
						: "=r"(pfr0)); \
				   ((pfr0 & ID_AA64PFR0_ASIMD_MASK) != 0xf); \
				})

#define cpu_supports_fpu()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_aa64pfr0_el1" \
						: "=r"(pfr0)); \
				   ((pfr0 & ID_AA64PFR0_FPU_MASK) != 0xf); \
				})

#define cpu_supports_el0_a32()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_aa64pfr0_el1" \
						: "=r"(pfr0)); \
				   ((pfr0 & ID_AA64PFR0_EL0_MASK) == ID_AA64PFR0_EL0_A32); \
				})

#define cpu_supports_el1_a32()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_aa64pfr0_el1" \
						: "=r"(pfr0)); \
				   ((pfr0 & ID_AA64PFR0_EL1_MASK) == ID_AA64PFR0_EL1_A32); \
				})

#define cpu_supports_el2_a32()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_aa64pfr0_el1" \
						: "=r"(pfr0)); \
				   ((pfr0 & ID_AA64PFR0_EL2_MASK) == ID_AA64PFR0_EL2_A32); \
				})

#define cpu_supports_el3_a32()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_aa64pfr0_el1" \
						: "=r"(pfr0)); \
				   ((pfr0 & ID_AA64PFR0_EL3_MASK) == ID_AA64PFR0_EL3_A32); \
				})

#define cpu_supports_el0()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_aa64pfr0_el1" \
						: "=r"(pfr0)); \
				   (pfr0 & ID_AA64PFR0_EL0_MASK); \
				})

#define cpu_supports_el1()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_aa64pfr0_el1" \
						: "=r"(pfr0)); \
				   (pfr0 & ID_AA64PFR0_EL1_MASK); \
				})

#define cpu_supports_el2()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_aa64pfr0_el1" \
						: "=r"(pfr0)); \
				   (pfr0 & ID_AA64PFR0_EL2_MASK); \
				})

#define cpu_supports_el3()	({ uint64_t pfr0; \
				   asm volatile("mrs %0, id_aa64pfr0_el1" \
						: "=r"(pfr0)); \
