...
#define VCPU_MAX_VGIC_LRS 16
#define VCPU_MAX_VGIC_APRS 4
...
struct arch_vcpu {
    ...
    /* GICv3 CPU interface (ICC_*_EL1). These are per-physical-CPU regs but
       each vCPU writes/reads them as if it owned its own GIC interface, so
       hyp must swap them on context switch. */
    struct {
        uint64_t pmr;       /* ICC_PMR_EL1 */
        uint64_t ctlr;      /* ICC_CTLR_EL1 */
        uint64_t igrpen1;   /* ICC_IGRPEN1_EL1 */
        uint64_t bpr1;      /* ICC_BPR1_EL1 */
        uint64_t sre_el1;   /* ICC_SRE_EL1 */
        bool     initialized;
    } gicc;
    ...
    struct {
        uint64_t hcr;
        uint64_t vmcr;
        uint64_t ap0r[VCPU_MAX_VGIC_APRS];
        uint64_t ap1r[VCPU_MAX_VGIC_APRS];
        uint64_t lr[VCPU_MAX_VGIC_LRS];
        uint32_t lr_count;
        bool     initialized;
    } vgic;
#if 0
