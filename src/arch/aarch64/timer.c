...
static u64 read_vmm_physical_ctrl() {
  return   mrs(cnthp_ctl_el2);
}
...
#define COUNTER_TIMER_HYPER_CTL_REG    cnthctl_el2 /* this ctrl not for timer, diffs with cnthp_ctl_el */
...
enable_irq();
    ...
    vmm_debug("debug: %lx\n", mrs(daif));
    vmm_info("status: %lx, offset:%lx\n", mrs(cntp_ctl_el0), read_offset());
