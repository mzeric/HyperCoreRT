...
/******************************************************************************
 * Intrusive doubly-linked-list and hash-list helpers used throughout
 * HyperCoreRT.  The interface follows the convention popularised by
 * the Linux kernel (struct list_head + list_add/del/for_each_entry,
 * struct hlist_head + hlist_add_head/...), which lets unrelated parts
 * of the code share one set of mental models.
 */



/*
 * Poison pointers handed out by list_del() and friends.  They are
 * deliberately non-canonical addresses so that any accidental
 * dereference faults immediately rather than corrupting a real list;
...
static inline int list_head_is_null(const struct list_head *list)
{
